...
-- TargetLanguage: VHDL
-- FoldingFactor: 24
-- TargetDirectory: C:\Users\idont\Documents\School\Vysoka\Matlab\Bakalarka\AAA_in13_coeff13_fullPrecision
-- TestBenchName: filter_tb_full
-- TestBenchStimulus: impulse step ramp chirp noise 
--
-- Filter Settings:
--
-- Filter Specifications: SRS3Faep-imsedlppcBtloi eenfPr sio ReciOa anrt ttde i e  o r  n       :::::     NLN08/o,. AwF6  p3  (ad  nsB  os   r    m    a    l    i    z    e    d         f    r    e    q    u    e    n    c    y    )     D-FNSLANDSISSOSNDNDROi-iutiruecneeutueueovs-lmanimnapcctamnmnuec-tbbeteoluttpteoeonrr-eelahrmetiiuermrmdfe-rrermai  oot aiai lt-    etnV nn  tntnMoe-So Ptoaa     oaoaow--tf hirtl IO  rtrtd T-r  ac ou nu   o oeMi-uS s  re pt  PrAr om-ce e   s up  r c  de-tc       tu  oPcA e -ut        t  druc  I-ri            omc  I-eo            d u  R- n              m   - s                 F-                   i-                   l-:::::::::::::::::::t-                   e-D4YNfssssssssssssfsr-i eoi111111113333la -r s x666666662244ot(-e   e,,,,,,,,,,,,our-c   d111111112222rre-t    333333336666 aa--                 tl-F    ------------ e)-o    >>>>>>>>>>>>    r                    m    [[[[[[[[[[[[         ------------    I    444444443311    I            2222    ,    44444444  88         ))))))))33      S            2211    e            ))22    c              88    o              ))    n                    d                    -                    O                    r                    d                    e                    r                                         S                    e                    c                    t                    i                    o                    n                    s
-- -------------------------------------------------------------
LIBRARY IEEE;
...
PACKAGE filter_tb_full_pkg IS

  -- Type Definitions
  TYPE filter_in_data_log_type IS ARRAY (0 TO 3435) OF std_logic_vector(15 DOWNTO 0);
...
END filter_tb_full_pkg;

PACKAGE BODY filter_tb_full_pkg IS
...
-- Counter to generate Addr.
    ...
    ELSIF clk'event and clk = '1' THEN
      IF rdenb = '1' THEN
        IF (addr = TO_UNSIGNED(3435, 12 )) THEN
          addr     <= addr; 
        ELSE
...
-- Done Signal generation.
    ...
    ELSIF (addr = TO_UNSIGNED(3435, 12 )) THEN
      done <= '1'; 
    ELSE
      done <= '0'; 
    END IF;
...
USE IEEE.std_logic_1164.all;
USE IEEE.numeric_std.ALL;
USE work.filter_tb_full_pkg.ALL;

PACKAGE filter_tb_full_data IS

  CONSTANT filter_in_data_log_force : filter_in_data_log_type;
  CONSTANT filter_out_expected : filter_in_data_log_type;

END filter_tb_full_data;

PACKAGE BODY filter_tb_full_data IS
...
USE work.filter_tb_full_data.ALL;

ENTITY filter_tb_full IS

END filter_tb_full;


ARCHITECTURE rtl OF filter_tb_full IS
  -- -------------------------------------------------------------
  ...
  CONSTANT MAX_ERROR_COUNT                : integer := 3436; -- uint32
  ...
  SIGNAL filter_in                        : std_logic_vector(15 DOWNTO 0); -- sfix16_En13
  SIGNAL filter_out                       : std_logic_vector(15 DOWNTO 0); -- sfix16_En13
  ...
  SIGNAL counter                          : unsigned(5 DOWNTO 0); -- ufix5
  ...
  SIGNAL holdData_filter_in               : std_logic_vector(15 DOWNTO 0); -- sfix16_En13
  ...
  SIGNAL int_delay_pipe                   : std_logic_vector(0 TO 47); -- boolean
  ...
  SIGNAL filter_out_ref                   : std_logic_vector(15 DOWNTO 0); -- sfix16_En13
  SIGNAL filter_out_dataTable             : std_logic_vector(15 DOWNTO 0); -- sfix16_En13
  SIGNAL filter_out_refTmp                : std_logic_vector(15 DOWNTO 0); -- sfix16_En13
  SIGNAL regout                           : std_logic_vector(15 DOWNTO 0); -- sfix16_En13
  SIGNAL check1_Done                      : std_logic; -- boolean
  --my signals
  signal GNT, RDY, RQ                     : std_logic;
...
BEGIN
  ...
  BEGIN
    IF reset = '1' THEN
      counter <= to_unsigned(1, 6);
    ELSIF clk'event AND clk = '1' THEN
      IF tbenb_dly = '1' THEN
        IF counter >= to_unsigned(45, 6) THEN --puvodni hodnota = 23, nová = 23 + 22 = 45
          counter <= to_unsigned(0, 6);
        ELSE
          counter <= counter + to_unsigned(1, 6);
        END IF;
      END IF;
    END IF; 
  END PROCESS slow_clock_enable;

  phase_1 <= '1' WHEN counter = to_unsigned(1, 6) AND tbenb_dly = '1' ELSE '0';
  ...
  BEGIN
    IF reset = '1' THEN
      int_delay_pipe(0 TO 47) <= (OTHERS => '0');
    ELSIF clk'event AND clk = '1' THEN 
      IF clk_enable = '1' THEN
        int_delay_pipe(0) <= rdEnb_phase_1;
        int_delay_pipe(1 TO 47) <= int_delay_pipe(0 TO 46);
      END IF;
    END IF; 
  END PROCESS ceout_delayLine;

  delayLine_out <= int_delay_pipe(47);
  ...
  BEGIN
    ...
    ELSIF clk'event and clk ='1' THEN
      IF filter_out_rdenb = '1' THEN
        IF (Is_X(filter_out) OR abs(signed(filter_out)  - signed(filter_out_expected(TO_INTEGER(filter_out_addr)))) > 15) THEN --pokud je filter_out undefined nebo je absolutní chyba > 15 jde o chybný výstup
